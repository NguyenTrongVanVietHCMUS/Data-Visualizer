...
std::vector<Node> MaxHeap::BuildNodeFromValue(const std::vector<std::string> &values)
{
    CalcPosition(nodes);
    return nodes;
}

void MaxHeap::HeapifyDown(Presentation &myPresentation, std::vector<int> highlightLines)
{
...
void MaxHeap::HeapifyUp(Presentation &myPresentation, std::vector<int> highlightLines)
{
    myPresentation.CopySetToLast(-1);
    myPresentation.SetStartAnimation(-1, 1);
    int curID = size - 1;
...
Presentation MaxHeap::CreateAnimation(const std::vector<Node> &nodes)
{
    Presentation create;
    { // Insert all nodes
...
void MaxHeap::ShiftDown(Presentation &myPresentation, std::vector<int> highlightLines, int curID)
{
    myPresentation.CopySetToLast(-1);
    myPresentation.SetStartAnimation(-1, 1);
    Node from, to;
...
void MaxHeap::ShiftUp(Presentation &myPresentation, std::vector<int> highlightLines, int curID)
{
    myPresentation.CopySetToLast(-1);
    myPresentation.SetStartAnimation(-1, 1);
    Node from, to;
...
void MaxHeap::BuildCreateAnimation()
{
...
int MaxHeap::FindPosition(std::string value)
{
...
void MaxHeap::InputDataFromFile()
{
    ClearAllData();
    auto HeapifyUp = [&]() -> void 
    {
...
SetOfAnimation MaxHeap::BasicStructure(const std::vector<Node> &nodes)
{
...
void MaxHeap::CalcPosition(std::vector<Node> &nodes, int curID, int height, float x, float y)
{
...
void MaxHeap::Push(std::string val)
{
    myPresentation.present = {BasicStructure(nodes)};
    auto AddValueToHeap = [&](std::string value) -> void 
    {
...
std::vector<int> MaxHeap::StringToVector(std::string listChar)
{
...
void MaxHeap::Update(int pos, std::string val)
{
    if (size > 0)
    {
